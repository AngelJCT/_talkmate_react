...
function GlassCard({ hide, setChatLog, setIsLoading, chatLog }) {
  ...
  const Card = ({ title, content }) => (
    <div
      ...
      className={`card backdrop-blur-md bg-gray-50 bg-opacity-30 bg-clip-padding border border-white border-opacity-20 p-4 rounded-xl shadow-xl w-full h-28 transition-transform duration-700 ease-in-out cursor-pointer ${
        activeCard === title ? "animate-custom-bounce" : ""
      }`}
    >
  ...
  return (
    <div
      className={`flex flex-col justify-end xs:mt-4 md:mt-0 ${
        hide ? "hide" : ""
      }`}
    >
      <div className="md:hidden mx-4 shadow-xl">
        <Slider arrows={false} {...settings}>
          <Card
            title="How can I tell someone"
            content="that I'm grateful for them in Japanese?"
          />
          <Card
            title="How can I introduce myself"
            content="in Japanese in different scenarios?"
          />
          <Card
            title="What is the difference between"
            content='"Ore wa" and "Watashi wa" when referring to myself?'
          />
          <Card
            title="Elaborate on how to refers to someone"
            content="in formal and casual occasions."
          />
        </Slider>
      </div>
